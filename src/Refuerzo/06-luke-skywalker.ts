...
import { PersonajeSW } from './Interefaces/personaje.interface';
...
(() =>{
    ...
    const endpoint_1 = '/people/2/';
    getRequest(`${SW_API}${endpoint_1}`).pipe(
        // Realizar los operadores respectivos aquí
        switchMap(respuesta => zip(
            of<PersonajeSW>(respuesta),
            getRequest(respuesta.species[0]
        ))),
        map(([persona, especie])=> ({ persona, especie}))
